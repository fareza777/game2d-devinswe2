...
namespace SmallScale.FantasyKingdomTileset
{
    ...
    public static class InputAdapter
    {
        public static Vector3 mousePosition
        {
            get
            {
                if (VirtualControlsActive)
                    return VirtualPointerScreen;
                Vector2 pos = Mouse.current?.position.ReadValue() ?? Vector2.zero;
                return new Vector3(pos.x, pos.y, 0f);
            }
        }

        public static bool mousePresent => VirtualControlsActive || Mouse.current != null;

        // Touch UI (MobileControls) and scripted probes drive input through these.
        public static bool VirtualControlsActive { get; set; }
        public static Vector2 VirtualMove { get; set; }
        public static Vector3 VirtualPointerScreen { get; set; }

        public static void SetVirtualKey(KeyCode key, bool pressed)
        {
            virtualKeys.TryGetValue(key, out var state);
            if (state.Pressed == pressed)
                return;
            state.Pressed = pressed;
            if (pressed) state.DownFrame = Time.frameCount; else state.UpFrame = Time.frameCount;
        ...
        {
            if (TryGetVirtualKey((KeyCode)((int)KeyCode.Mouse0 + button), out var vdown, MouseButtonState.Down))
                return vdown;
        ...
        {
            if (TryGetVirtualKey((KeyCode)((int)KeyCode.Mouse0 + button), out var vpressed, MouseButtonState.Pressed))
                return vpressed;
            var control = GetMouseButtonControl(button);
        ...
        {
            if (TryGetVirtualKey((KeyCode)((int)KeyCode.Mouse0 + button), out var vup, MouseButtonState.Up))
                return vup;
            var control = GetMouseButtonControl(button);
        ...
        {
            if (TryGetVirtualKey(key, out var vkdown, MouseButtonState.Down))
                return vkdown;
        ...
        {
            if (TryGetVirtualKey(key, out var vkpressed, MouseButtonState.Pressed))
                return vkpressed;
        ...
        {
            if (TryGetVirtualKey(key, out var vkup, MouseButtonState.Up))
                return vkup;
        ...
        public static float GetAxisRaw(string axisName)
        {
            ...
            Vector2 virtualMove = VirtualControlsActive ? VirtualMove : Vector2.zero;

            switch (axisName)
            {
                case "Horizontal":
                    return ComposeAxis(ReadNegativeHorizontal(), ReadPositiveHorizontal(), gamepadStick.x + virtualMove.x);
                case "Vertical":
                    return ComposeAxis(ReadNegativeVertical(), ReadPositiveVertical(), gamepadStick.y + virtualMove.y);
        ...
        private static bool TryGetVirtualKey(KeyCode key, out bool state, MouseButtonState desiredState)
        {
            state = false;
            if (!VirtualControlsActive || !virtualKeys.TryGetValue(key, out var vs))
                return false;
            state = desiredState switch
            {
                MouseButtonState.Down => vs.Pressed && vs.DownFrame == Time.frameCount,
                MouseButtonState.Up => !vs.Pressed && vs.UpFrame == Time.frameCount,
                _ => vs.Pressed
            };
            return true;
        }

        private static readonly Dictionary<KeyCode, VirtualKeyState> virtualKeys = new Dictionary<KeyCode, VirtualKeyState>();

        private struct VirtualKeyState
        {
            public bool Pressed;
