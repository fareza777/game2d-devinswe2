...
namespace SmallScaleInc.CharacterCreatorFantasy
{
    ...
    public class GenericTopDownController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // Touch joystick: absolute screen-space direction, magnitude throttles speed below.
            Vector2 stickMove = Input.VirtualControlsActive ? Input.VirtualMove : Vector2.zero;
            if (stickMove.sqrMagnitude > 0.01f)
            {
                rawMove += stickMove;
                hasClickTarget = false;
            }
            float moveStrength = Mathf.Clamp01(rawMove.magnitude);
            ...
            float speed = (walkHold ? walkSpeed : (runSpeed * mountMultiplier)) * moveStrength;
